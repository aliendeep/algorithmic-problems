...
that can be contained ( Yes, we know maximum area instead of maximum volume 
...
Explanation : 5 and 3 are distance 2 apart. So size of the base = 2. Height of 
container = min(5, 3) = 3. 
So total area = 3 * 2 = 6
*/
...
public class Solution {
  public int maxArea(ArrayList<Integer> heights) {
      ...
      while(l < r){
          int a = heights.get(l);
          int b = heights.get(r);
          area = Math.max(area, Math.min(a, b)*(r-l));
          if(a < b)
              l++;
          else if(a > b)
              r--;
