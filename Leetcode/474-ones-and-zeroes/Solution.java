/*
In the computer world, use restricted resource you have to generate maximum 
benefit is what we always want to pursue.

For now, suppose you are a dominator of m 0s and n 1s respectively. On the other 
hand, there is an array with strings consisting of only 0s and 1s.

Now your task is to find the maximum number of strings that you can form with 
given m 0s and n 1s. Each 0 and 1 can be used at most once.

Note:
The given numbers of 0s and 1s will both not exceed 100
The size of given string array won't exceed 600.
Example 1:
Input: Array = {"10", "0001", "111001", "1", "0"}, m = 5, n = 3
Output: 4

Explanation: This are totally 4 strings can be formed by the using of 5 0s and 
3 1s, which are “10,”0001”,”1”,”0”
Example 2:
Input: Array = {"10", "0", "1"}, m = 1, n = 1
Output: 2

Explanation: You could form "10", but then you'd have nothing left. Better form 
"0" and "1".
*/
// O(kmn)
public class Solution {
    int[][][] dp;
    int[] w0;
    int[] w1;
    public int getValue(int cap1, int cap2, int i){
        if(cap1 < 0 || cap2 < 0 || i < 0)   return 0;
        if(dp[i][cap1][cap2] != -1)         return dp[i][cap1][cap2];
        
        // don't take
        dp[i][cap1][cap2] = getValue(cap1, cap2, i-1);
        if(w0[i] <= cap1 && w1[i] <= cap2){
            // take this
            dp[i][cap1][cap2] = Math.max(dp[i][cap1][cap2], 
                                getValue(cap1 - w0[i], cap2 - w1[i], i-1) + 1);
        }
        return dp[i][cap1][cap2];
    }

    public int findMaxForm(String[] strs, int m, int n) {
        int len = strs.length;
        if(len == 0)
            return 0;
        w0 = new int[len];        
        w1 = new int[len];
        
        for(int i=0; i<len; ++i){
            String s = strs[i];
            for(int j=0; j<s.length(); ++j){
                if(s.charAt(j) == '1')
                    w1[i]++;
                else
                    w0[i]++;
            }
        } 
        dp = new int[len][m+1][n+1];
        for(int[][] x : dp){
            for(int[] y : x)
                Arrays.fill(y, -1);
        }
        return getValue(m, n, len-1);
    }
}

